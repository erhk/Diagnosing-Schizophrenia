...
#Save plot
RocplotSave <- ggsave("Rocplot.pdf", plot(rocCurve, legacy.aex = TRUE), device = "pdf", path = "~/R/R - Datascripts/Assignment 3/RocCurve plots", limitsize = FALSE)
...
for(i in 1:k){
  ...
  trainRoc[n] = trainRocCurve$auc

  TrainCross <- data.frame(trainSens, trainSpec, trainPPV, trainNPV, trainACC, trainKappa,    trainRoc)  
  
  #Plot

  #RocplotLoop
#Save plot
  RocplotSave <- ggsave("Train_RocplotLoop.pdf", plot(trainRocCurve, legacy.aex = TRUE), device = "pdf",   path = "~/R/R - Datascripts/Assignment 3/RocCurve plots", limitsize = FALSE)
...
#_______________________________________________________________________________________
 ...
 testRoc[n] = testRocCurve$auc
 
 
 TestCross <- data.frame(testSens, testSpec, testPPV, testNPV, testACC, testKappa, testRoc)
 #RocplotLoop
#Save plot
  RocplotSave <- ggsave("Test_RocplotLoop.pdf", plot(testRocCurve, legacy.aex = TRUE), device = "pdf",   path = "~/R/R - Datascripts/Assignment 3/RocCurve plots", limitsize = FALSE)
...
#Tidy up data
CrossVal2 <- dplyr::select(CrossVal, trainSens:trainRoc) %>%
  gather("Train", "Train Values")

#Tidy up data
CrossVal3 <- dplyr::select(CrossVal, testSens:testRoc) %>%
...
modelName <- lm(diagnosis~iv, data=schiData) 

glmModel <- glmer(diagnosis ~ 1 + dv + (1|study), schiData, family = "binomial")
  

dvlist <- names(schiData)[8:15]
dvlist
?base
model1 <- lapply(dvlist, function(x) {
    glmer(substitute(i~dv+ (1|study), list(i = as.name(x))), data = schiData)})
