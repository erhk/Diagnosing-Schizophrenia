...
p_load(ggplot2,pastecs,rmarkdown,tidyr,dplyr,QuantPsyc, VIF,stringr,tidytext,DHARMa,lme4,psych,MuMIn,tidyverse,magrittr,Metrics,simr,readtext,crqa,readr,Hmisc,lmerTest,caret,gtools) 
...
#Save plot
RocplotSave <- ggsave("Rocplot.pdf", plot(rocCurve, legacy.aex = TRUE), device = "pdf", path = "E:/R/R scripts/Diagnosing-Schizophrenia-master/RocCurve plots", limitsize = FALSE)
...
#My model build
glmModel <- glmer(diagnosis ~ 1 + dv + (1|study), schiData, family = "binomial")

#Function to do glmer on every predictor. 
# model1 <- lapply(dv, function(x) {
#     glmer(substitute(diagnosis~dv+ (1|study), list(dv = as.name(x))), data = schiData)})
# model1 

#Function does glmer and confusion matrix and calculates AUC on each predictor.
model2 <- lapply(dv, function(x) {
  glmModel <- glmer(substitute(diagnosis~dv+ (1|study), list(dv = as.name(x))), data = schiData, family = "binomial")
  print(glmModel)
  schiData$PredictionsPerc1 <- predict(glmModel, schiData)
  schiData$Predictions1[schiData$PredictionsPerc1>0]="0"
  schiData$Predictions1[schiData$PredictionsPerc1<=0]="1"

  schiDatapredModel<- confusionMatrix(data = schiData$Predictions1, reference = schiData$diagnosis, positive = "1")

  ModelRocCurve <- roc(response = schiData$diagnosis, predictor = schiData$PredictionsPerc1)
  print(ModelRocCurve)
})
