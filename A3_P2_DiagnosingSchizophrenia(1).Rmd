...
```{r}
setwd("E:/R/R scripts/Diagnosing-Schizophrenia-master")
#Load library. #alternative load package 
p_load(ggplot2,pastecs,rmarkdown,tidyr,dplyr,QuantPsyc, VIF,stringr,tidytext,DHARMa,lme4,psych,MuMIn,tidyverse,magrittr,Metrics,simr,readtext,crqa,readr,Hmisc,lmerTest,caret,gtools,pROC)library(pacman)
...
library(lme4);library(lmerTest);library(psych);library(MuMIn);library(tidyverse);library(magrittr);library(Metrics);library(caret);library(simr);library(readtext);library(crqa);library(readr);library(gtools);library(lattice);library(nlme)
...
for(i in 1:k){
  ...
  #RocplotLoop
#Save plot
  RocplotSave <- ggsave("Train_RocplotLoop.pdf", plot(trainRocCurve, legacy.aex = TRUE), device = "pdf",   path = "E:/R/R scripts/Diagnosing-Schizophrenia-master/RocCurve plots", limitsize = FALSE)
...
#_______________________________________________________________________________________
 ...
 #RocplotLoop
#Save plot
  RocplotSave <- ggsave("Test_RocplotLoop.pdf", plot(testRocCurve, legacy.aex = TRUE), device = "pdf",   path = "E:/R/R scripts/Diagnosing-Schizophrenia-master/RocCurve plots", limitsize = FALSE)
...
##First model thingy ___ WRONG - IGNORE - DOESN'T WORK ___ Just keeping it for note
```{r}
#Make list of predictors to run through model in loop

schiData$SUBJ <- as.numeric(factor(schiData$participant))
#number of folds
k = 10
schi_folds <- createFolds(unique(schiData$participant, k=k, list = T, ReturnTrain = T)) 
schiData$SUBJ <- as.numeric(factor(schiData$participant))
dv <- names(schiData)[8:15]
...
#for (feature in dv){
  ...
  # N = 1
    ...
    #Make the string for the string for the model
     #stringModel = paste("diagnosis ~ scale(", dv, ") + (1|study)", sep = "")

#}
#My model build
#glmModel <- glmer(diagnosis ~ 1 + scale(dv) + (1|study), schiData, family = "binomial")
#Function does glmer and confusion matrix and calculates AUC on each predictor.
#for(schi_folds in 1:k) { 
models <- lapply(dv, function(x) { 
    #dv <- names(schiData)[8:15]
    test2 <- subset(schiData, (SUBJ  %in% schi_folds[[i]])) #find all subjects in schiData, look in all folders.
    train2<- subset(schiData, !(SUBJ  %in% schi_folds[[i]])) #Do the opposite    
  
#TRAIN model on train data_______________________________________________________________
    #model won't work.
    glmModel2 <- glmer(substitute(i ~ 1 + scale(dv) + (1|study), list(i = as.name(x))), train2, family = "binomial")
    #stringModel = paste("diagnosis ~ scale(", dv, ") + (1|study)", sep = "")

    #glmModel2 = glmer(stringModel, train2, family = binomial)
    print(glmModel2)
    #train2$PredictionsPerc1 <- predict(glmModel2, train2)
    train2$PredictionsPerc1 <- predict(glmModel2, train2)
    train2$Predictions1[train2$PredictionsPerc1>0]="0"
    train2$Predictions1[train2$PredictionsPerc1<=0]="1"
    
    train2DatapredModel <- (confusionMatrix(data = train2$Predictions1, reference = train2$diagnosis, positive = "1"))
    print(train2DatapredModel)
  
    
    Train2ModelRocCurve <- roc(response = train2$diagnosis, predictor = train2$PredictionsPerc1)
    print(Model2RocCurve)
   
#Test model on test data_______________________________________________________________

  
    test2$PredictionsPerc1 <- predict(glmModel2, test2)
  
    test2$Predictions1[test2$PredictionsPerc1>0]="0"
    test2$Predictions1[test2$PredictionsPerc1<=0]="1"
  
    test2DatapredModel <- (confusionMatrix(data = test2$Predictions1, reference = test2$diagnosis, positive = "1"))
    print(test2DatapredModel)

  
    Test2ModelRocCurve <- roc(response = test2$diagnosis, predictor = test2$PredictionsPerc1)
    print(Test2ModelRocCurve)
...
#Second Loop thingy ___ Because my function loop sucks and is wrong ___________
...
``` 
Train rqa_RR auc = 0.54345793 
Test rqa_RR auc = 0.53716645
...
#Make models
stringMultiple = c("diagnosis ~ 1 + scale(rqa_RR) + scale(rqa_NRLINE) + (1|study)",
"diagnosis ~ 1 + scale(rqa_RR) + scale(rqa_NRLINE) + scale(rqa_L) + (1|study)",
"diagnosis ~ 1 + scale(rqa_RR) + scale(rqa_NRLINE) + scale(rqa_L) + (1|study)",
"diagnosis ~ 1 + scale(rqa_RR) * scale(rqa_DET) + rqa_L + (1|study)")


modelName <- c("Q3_model1", "Q3_model2", "Q3_model3", "Q3_model4")
